...
<i>Explanation for (c).</i> Suppose \( r(x) \) and \( s(x) \) are polynomials of degree \(q\) and \(n-q\), respectively. We show that if
\( s(x) \) is a palindrome then \(r(x)\) is also one. We have

\begin{align*}
r(x) &= \frac{p(x)}{s(x)} \\
r(x) &= \frac{x^np(1/x)}{x^{n-q}s(1/x)} \\
r(x) &= x^q r(1/x)
\end{align*}
...
<br>
Let \( (x_0,y_0) \) be a particular solution to the first equation \( ax+by = c\). Without
loss of generality, we may assume that \( (x_0,y_0) \) is such that \( (x_0 + kb,y_0-ka) \) is a solution for \( k \in \{0,\ldots,m-1\} \).


<br>
This means that \( (x_0+b+kb,y_0-ka) \) is a solution to the second equation for \(k\in \{-1,0,1,\ldots,m-1\}\).
Hence, there are \(m+1\) solutions.
</p>
