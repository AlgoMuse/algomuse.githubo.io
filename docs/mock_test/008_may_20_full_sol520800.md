...
<details open><summary>Sol.</summary>
<b>Ans.</b> e. \(f(x)=x\) is a bijection. \(f(x)=|x|\) is neither one-one nor onto.
</details>
...
(i) We have \((m+1) p q=(p+1)(q+1)\), which reduces to \((p-m)(q-m)=m(m+1)\). <br><br>
...
Since \(m\) and \(m+1\) are coprime, the number of factors of \(m(m+1)\) is equal to \(\tau(m) \tau(m+1)\), and so the number of representations is equal to \(\frac{1}{2} \tau(m) \tau(m+1)\).<br><br>

(iii) \( \frac{\Phi(26)\cdot\Phi(27)}{2} = \frac{12\cdot18}{2} = 108 \).
