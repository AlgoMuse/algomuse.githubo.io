...
<p>
B1. Consider a coordinate grid made up of horizontal and vertical lines of the form \(x=k\) and \(y=k\), where \(k\in \mathbb{Z}\).
...
<p>B2. Suppose \( f(x)=\cos^{-1}(\cos (\pi x)) \). Find the value of \( f(2.6) \).</p>
...
<p>
B4. Find all the positive integer solutions to the following equation:
...
<p>
B5. Prove that there are no integers \(x>0\) and \(n>0\) such that  \( x(x+1) = 2021^n \).
</p>
